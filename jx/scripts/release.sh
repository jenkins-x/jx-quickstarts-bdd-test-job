...
mkdir -p $DUMMY_DIR
export KUBECONFIG=$DUMMY_DIR/context
...
jx step bdd -b  --provider=gke --git-provider=ghe --git-provider-url=https://github.beescloud.com --git-username dev1 --git-api-token $GHE_CREDS_PSW --default-admin-password $JENKINS_CREDS_PSW --no-delete-app --no-delete-repo --tests install --tests $TESTS --ignore-fail  2>&1 | tee /tmp/build-log.txt
...
jx step stash -c tests  --basedir "/home/jenkins/go/jenkins-x/bdd-jx/reports" -p "/home/jenkins/go/jenkins-x/bdd-jx/reports/*" --bucket-url gs://jenkins-x-tests --to-path logs/quickstarts/$BUILD_NUMBER
